...
void Finder::Render() {
    ...
    if (ImGui::Begin("Finder", &this->is_shown)) {
        ...
        if (ImGui::BeginChild("##header_bar",
                              ImVec2(0, header_bar_height),
                              false,
...
#pragma endregion
                ...
                ImGui::SetCursorScreenPos(
                    ImVec2(spinnerLocation.x + REFRESH_BUTTON_SIZE + spacing.x, spinnerLocation.y));
                auto doSearch = ImGui::InputText("##search_input",
                                                 this->state->query,
            ...
            int grid_col_count = static_cast<int>(std::floor(win_size.x / GRID_ITEM_SIZE)) - 2;

            if (ImGui::BeginChild("##search_result_area",
                                  ImVec2(0, 0),
                                  false,
                                  ImGuiWindowFlags_NoNav | ImGuiWindowFlags_AlwaysUseWindowPadding)) {
                ...
                for (const auto &[ep, results]: this->state->items) {
                    ...
                    bool ep_open = ImGui::CollapsingHeader(
                        std::format("{}", results[0].endpoint).c_str(),
                    ...
                    if (ep_open) {
                        ...
                        if (current_y + ep_grid_height > scroll_y && current_y < scroll_y + window_height) {
                            int first_visible_row = static_cast<int>(scroll_y / GRID_ITEM_SIZE);
                            int num_visible_rows = static_cast<int>(win_size.y / GRID_ITEM_SIZE);

                            // Add space for rows above the first visible row
                            ImGui::Dummy(ImVec2(0, static_cast<float>(first_visible_row) * GRID_ITEM_SIZE));

                            int last_visible_element = std::min((first_visible_row + num_visible_rows) * grid_col_count,
                            ...
                            if (ImGui::BeginTable(std::format("Table: {}", ep).c_str(), grid_col_count)) {
                                ...
                                for (int index = first_visible_row * grid_col_count; index <= last_visible_element;
                                     index++) {
                            ...
                            ImGui::Dummy(ImVec2(
                                0,
                                ep_grid_height - std::ceil(
                                    static_cast<float>(last_visible_element) / static_cast<float>(grid_col_count)
                                )
                            ));
                        }
            ...
            if (ImGui::BeginPopupModal("API Key",
